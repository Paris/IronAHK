...
namespace IronAHK.Rusty
{
    partial class Core
    {
        ...
        public static void IniDelete(string file, string section, string key = null)
        {
            ...
            {
                ErrorLevel = Windows.WritePrivateProfileString(section, key.Length == 0 ? null : key, null, file) ? 0 : 1;
                return;
            }
        ...
        public static void IniRead(out string result, string file, string section, string key, string error = "ERROR")
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
                ...
                Windows.GetPrivateProfileString(section, key, error, buf, (uint)buf.Capacity, file);
                result = buf.ToString();
            }
        ...
        public static void IniWrite(string value, string file, string section, string key)
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
                ErrorLevel = Windows.WritePrivateProfileString(section, key, value, file) ? 0 : 1;
                return;
            }
