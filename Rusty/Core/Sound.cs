    ...
    partial class Core
    {
        ...
        public static void SoundBeep(int Frequency, int Duration)
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                Windows.Sound.Beep((uint)Frequency, (uint)Duration);
        ...
        /// <para>If a device has more than one instance of ComponentType (two of type LINE, for example), usually the first contains the playback settings and the second contains the recording settings. To access an instance other than the first, append a colon and a number to this parameter. For example: Analog:2 is the second instance of the analog component.</para>
        /// </param>
        ...
        public static void SoundGetWaveVolume(out int OutputVar, int DeviceNumber)
        {
            ...
            uint vol = 0;
            Windows.Sound.waveOutGetVolume(new IntPtr(DeviceNumber), out vol);
            OutputVar = (int)vol;
        }
