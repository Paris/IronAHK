...
namespace IronAHK.Scripting
{
    partial class Emit
    {
        string Operator(CodeBinaryOperatorType op)
        {
            switch (op)
            {
                case CodeBinaryOperatorType.Assign: return new string(new char[] { Parser.AssignPre, Parser.Equal });
        ...
        string ScriptOperator(Script.Operator op)
        {
            switch (op)
            {
                ...
                case Script.Operator.Assign: return new string(new char[] { Parser.AssignPre, Parser.Equal });
                ...
                case Script.Operator.GreaterThanOrEqual: return new string(new char[] { Parser.Greater, Parser.Equal });
                case Script.Operator.IdentityEquality: return new string(Parser.Equal, 2);
                case Script.Operator.IdentityInequality: return new string(new char[] { Parser.Not, Parser.Equal, Parser.Equal });
                case Script.Operator.Increment: return new string(Parser.Add, 2);
                case Script.Operator.LessThan: return Parser.Less.ToString();
                case Script.Operator.LessThanOrEqual: return new string(new char[] { Parser.Less, Parser.Equal });
                ...
                case Script.Operator.ValueInequality: return new string(new char[] { Parser.Not, Parser.Equal });
